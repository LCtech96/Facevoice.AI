...
export function TrelloKanbanBoard({
  ...
  return (
    <div className={cn("flex gap-4 overflow-x-auto pb-4 rounded-2xl bg-[var(--background)] p-6 border border-[var(--border-color)]", className)}>
      {columns.map((column) => {
        ...
        return (
          <div
            ...
            className={cn(
              ...
              "bg-[var(--background-secondary)]/50 border-2",
              isDropActive ? "border-[var(--accent-blue)]/50 border-dashed bg-[var(--accent-blue)]/5" : "border-transparent",
            )}
          >
            <div className="mb-3 flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-[var(--background-secondary)] px-2 py-0.5 text-xs font-medium text-[var(--text-secondary)]">
                  {column.tasks.length}
                </span>
              </div>
            </div>
              ...
              {column.tasks.map((task) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      "cursor-grab rounded-lg border border-[var(--border-color)] bg-[var(--card-background)] p-3 shadow-sm transition-all duration-150",
                  ...
                  >
                    ...
                    <h3 className={cn("text-sm font-medium text-[var(--text-primary)]", task.description && "mb-1")}>
                      {task.title}
                    </h3>

                    {task.description && <p className="mb-2 text-xs text-[var(--text-secondary)]">{task.description}</p>}

                    {task.assignee && (
                      <div className="flex justify-end">
                        <div className="flex h-7 w-7 items-center justify-center rounded-full bg-[var(--accent-blue)] text-[11px] font-semibold text-white">
                          {task.assignee}
                        </div>
                      </div>
                    )}
              ...
              {allowAddTask && (
                <>
                  {addingCardTo === column.id ? (
                    <div className="rounded-lg border border-[var(--border-color)] bg-[var(--card-background)] p-3 shadow-sm">
                      <input
                        ref={inputRef}
                        type="text"
                        value={newCardTitle}
                        ...
                        className="mb-2 w-full border-none bg-transparent text-sm text-[var(--text-primary)] outline-none placeholder:text-[var(--text-secondary)]"
                      />
                      <div className="flex gap-2">
                        <button
                          onClick={() => handleAddCard(column.id)}
                          className="rounded bg-[var(--accent-blue)] px-3 py-1.5 text-xs font-medium text-white transition-colors hover:bg-[var(--accent-blue)]/90"
                        >
                          Add Card
                        </button>
                        <button
                          ...
                          className="rounded p-1.5 text-[var(--text-secondary)] transition-colors hover:bg-[var(--background-secondary)] hover:text-[var(--text-primary)]"
                          aria-label="Cancel"
                        >
                    ...
                    <button
                      ...
                      className="flex w-full items-center justify-center gap-1 rounded-lg p-2 text-sm text-[var(--text-secondary)] transition-colors hover:bg-[var(--background-secondary)] hover:text-[var(--text-primary)]"
                    >
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
