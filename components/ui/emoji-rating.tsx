...
export function RatingInteraction({ onChange, className }: RatingInteractionProps) {
  ...
  return (
    <div className={cn("flex flex-col items-center gap-6 p-8 bg-[var(--background)] rounded-2xl border border-[var(--border-color)]", className)}>
      ...
      <div className="relative h-7 w-32">
        ...
        >
          <span className="text-sm font-medium text-[var(--text-secondary)]">Rate us</span>
        </div>
        ...
        {ratingData.map((item, i) => (
          ...
          >
            <span className="text-sm font-semibold tracking-wide text-[var(--text-primary)]">{item.label}</span>
          </div>
        ))}
      </div>
    </div>
