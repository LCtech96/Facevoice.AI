...
import { CheckIcon, ArrowRightIcon, Phone, MessageSquare } from "lucide-react"
...
const steps: Step[] = [
  { id: 1, label: "Nome", field: "name", placeholder: "Il tuo nome completo" },
  { id: 2, label: "Numero WhatsApp", field: "whatsapp", placeholder: "+39 123 456 7890" },
  { id: 3, label: "Servizio Richiesto", field: "service", placeholder: "Descrivi il servizio o motivo per cui stai scrivendo" },
]
...
export function WhatsAppContactForm() {
  ...
  const handleSendWhatsApp = () => {
    ...
    const message = `*Nuova Richiesta da Facevoice AI*\n\n` +
      `*Nome:* ${formData.name}\n` +
      `*Numero WhatsApp:* ${formData.whatsapp}\n` +
      `*Servizio Richiesto:*\n${formData.service}\n\n` +
      `_Richiesta inviata il ${new Date().toLocaleString('it-IT')}_`
        ...
        <div className="relative overflow-hidden rounded-2xl border border-border/50 bg-gradient-to-br from-background via-background to-muted/20 p-12 backdrop-blur">
          ...
          <div className="relative flex flex-col items-center gap-4 animate-in fade-in zoom-in-95 duration-700">
            ...
            <div className="space-y-1 text-center">
              <h2 className="text-xl font-medium tracking-tight text-balance">Dati completati!</h2>
              <p className="text-sm text-muted-foreground/80">Clicca il pulsante per inviare via WhatsApp</p>
            </div>
            <Button
              onClick={handleSendWhatsApp}
              className="w-full h-12 group relative transition-all duration-300 hover:shadow-lg hover:shadow-foreground/5 mt-4 bg-green-500 hover:bg-green-600"
            >
