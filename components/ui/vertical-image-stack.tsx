...
export function VerticalImageStack() {
  ...
  return (
    <div className="relative flex h-[600px] w-full items-center justify-center overflow-hidden bg-[var(--background)] rounded-2xl border border-[var(--border-color)]">
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute left-1/2 top-1/2 h-[600px] w-[600px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[var(--text-primary)]/[0.02] blur-3xl" />
      </div>
      ...
      <div className="relative flex h-[500px] w-[320px] items-center justify-center" style={{ perspective: "1200px" }}>
        {images.map((image, index) => {
          ...
          return (
            ...
            >
              <div
                className="relative h-[420px] w-[280px] overflow-hidden rounded-3xl bg-[var(--card-background)] ring-1 ring-[var(--border-color)]/20"
                style={{
                  boxShadow: isCurrent
              ...
              >
                <div className="absolute inset-0 rounded-3xl bg-gradient-to-b from-[var(--text-primary)]/10 via-transparent to-transparent" />
                ...
                <div className="absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-[var(--background)]/60 to-transparent" />
              </div>
            </motion.div>
          )
        })}
        ...
        {images.map((_, index) => (
          <button
            ...
            className={`h-2 w-2 rounded-full transition-all duration-300 ${
              index === currentIndex ? "h-6 bg-[var(--text-primary)]" : "bg-[var(--text-primary)]/30 hover:bg-[var(--text-primary)]/50"
            }`}
      ...
      <div className="absolute left-8 top-1/2 -translate-y-1/2">
        <div className="flex flex-col items-center">
          <span className="text-4xl font-light text-[var(--text-primary)] tabular-nums">
            {String(currentIndex + 1).padStart(2, "0")}
          </span>
          <div className="my-2 h-px w-8 bg-[var(--text-primary)]/20" />
          <span className="text-sm text-[var(--text-secondary)] tabular-nums">{String(images.length).padStart(2, "0")}</span>
        </div>
      </div>
    </div>
  )
