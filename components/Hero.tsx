...
export default function Hero() {
  return (
    <section className="min-h-0 md:min-h-[55vh] flex items-center relative pt-4 pb-8 md:pt-4 px-4 sm:px-6 bg-black overflow-x-hidden">
      ...
      <div className="container mx-auto max-w-6xl z-10 relative w-full">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-12 items-center py-4 md:py-10">
          <motion.div
              ...
              {services.map((service, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="flex items-start gap-2 p-2.5 sm:p-3 rounded-xl bg-white/5 border border-white/10 backdrop-blur-sm"
                  >
                    <div className="flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 rounded-lg bg-[var(--accent-blue)]/20 flex items-center justify-center">
                      <Icon className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--accent-blue)]" />
                    </div>
                    <div className="text-left min-w-0 flex-1">
                      <p className="text-[11px] sm:text-sm font-semibold text-white leading-tight">{service.title}</p>
                      <p className="text-[10px] sm:text-xs text-gray-400 leading-snug mt-0.5">{service.description}</p>
                    </div>
                  </motion.div>
                )
              })}
          ...
          <motion.div
            ...
            className="flex justify-center lg:justify-end order-2 px-2 sm:px-0"
          >
